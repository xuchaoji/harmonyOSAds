...
#### 约束与限制
  ...
  - 在[WebviewController](api/arkts-apis-webview-webviewcontroller.md)类中，增加开启/关闭广告过滤特性的接口[enableAdsBlock()](api/arkts-apis-webview-webviewcontroller.md)，支持Web实例级的特性开关。

  - 新增[AdsBlockManager](api/arkts-apis-webview-adsblockmanager.md)全局单例类，提供自定义广告过滤配置、控制网站级特性开关的能力。

  - Web实例上提供了[onAdsBlocked()](api/arkts-basic-components-web-attributes.md)回调通知方法，支持将拦截信息通知到上层应用。

  - [AdsBlockManager](api/arkts-apis-webview-adsblockmanager.md)接口[setAdsBlockRules()](api/arkts-apis-webview-adsblockmanager.md)接口仅能设置一份自定义配置，此配置会持久化，应用冷启动无需重新配置，可避免每次冷启动配置规则都触发广告过滤配置的编译解析。

  - [AdsBlockManager](api/arkts-apis-webview-adsblockmanager.md)接口[addAdsBlockDisallowedList()](api/arkts-apis-webview-adsblockmanager.md)、[removeAdsBlockDisallowedList()](api/arkts-apis-webview-adsblockmanager.md)、[clearAdsBlockDisallowedList()](api/arkts-apis-webview-adsblockmanager.md)、[addAdsBlockAllowedList()](api/arkts-apis-webview-adsblockmanager.md)、[removeAdsBlockAllowedList()](api/arkts-apis-webview-adsblockmanager.md)、[clearAdsBlockAllowedList()](api/arkts-apis-webview-adsblockmanager.md)操作的数据不会持久化，应用冷启动需要重新设置。

  - 如果1个Web实例启用了广告过滤特性，但未调用[AdsBlockManager](api/arkts-apis-webview-adsblockmanager.md)接口[addAdsBlockDisallowedList()](api/arkts-apis-webview-adsblockmanager.md)、[removeAdsBlockDisallowedList()](api/arkts-apis-webview-adsblockmanager.md)、[clearAdsBlockDisallowedList()](api/arkts-apis-webview-adsblockmanager.md)、[addAdsBlockAllowedList()](api/arkts-apis-webview-adsblockmanager.md)、[removeAdsBlockAllowedList()](api/arkts-apis-webview-adsblockmanager.md)、[clearAdsBlockAllowedList()](api/arkts-apis-webview-adsblockmanager.md)配置disallowedlist和allowedlist数据，则默认所有网站均启用广告过滤。
...
### 开启广告过滤
     ...
     应用可以通过AdsBlockManager提供的[setAdsBlockRules()](api/arkts-apis-webview-adsblockmanager.md)接口设置自定义的easylist过滤规则，并通过Web组件的[enableAdsBlock()](api/arkts-apis-webview-webviewcontroller.md)接口使能广告过滤特性。
...
```
     如果存在内置的easylist规则文件，[setAdsBlockRules()](api/arkts-apis-webview-adsblockmanager.md)接口的replace参数可用于设置规则文件的使用策略，replace为true表示不使用内置的easylist规则文件，replace为false表示自定义规则和内置的规则将会同时工作，如果发现内置规则与自定义规则冲突，可使用replace=true禁用内置规则效果。

     设置的自定义规则文件在应用进程内对所有的Web组件生效，是一个应用级全局配置文件，并将持久化，应用重启后可继续工作。

### 关闭特定域名页面的广告过滤

     在Web组件的广告过滤开关开启后，应用有时候会期望关闭一些特定页面的广告过滤功能，除了可以使用自定义的easylist规则，AdsBlockManager还提供了[addAdsBlockDisallowedList()](api/arkts-apis-webview-adsblockmanager.md)接口完成此功能。
    ...
    Column() {
      Row() {
        Flex() {
          ...
          .onClick(() => {
            let arrDomainSuffixes = new Array();
...
```
     [addAdsBlockDisallowedList()](api/arkts-apis-webview-adsblockmanager.md)接口将域名设置到AdsBlockManager的DisallowedList中，下次页面加载时会使用网页url和DisallowedList中的域名进行后缀匹配，匹配成功则不会对此页面进行广告过滤。此外，还提供了[addAdsBlockAllowedList()](api/arkts-apis-webview-adsblockmanager.md)接口配合DisallowedList进行域名设置，控制是否开启广告过滤。
     ...
     例如，应用想要开启域名为'news.example.com'和'sport.example.com'的广告过滤，但需要关闭'example.com'的其他域名下网页的广告过滤，就可以先使用[addAdsBlockDisallowedList()](api/arkts-apis-webview-adsblockmanager.md)接口添加'example.com'域名到DisallowedList，再使用[addAdsBlockAllowedList()](api/arkts-apis-webview-adsblockmanager.md)接口添加'news.example.com'和'sport.example.com'域名。
...
```
     ...
     如果Web组件未通过[enableAdsBlock()](api/arkts-apis-webview-webviewcontroller.md)接口开启广告过滤功能，上述接口设置在此Web组件中将不起作用。

### 收集广告过滤的信息

     在Web组件的广告过滤开关开启后，访问的网页如果发生了广告过滤，会通过Web组件的[onAdsBlocked()](api/arkts-basic-components-web-attributes.md)回调接口通知到应用，应用可根据需要进行过滤信息的收集和统计。
